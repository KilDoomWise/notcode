    ...
    handler: async () => {
        try {
            ...
            return okJson({
                ...
                server: { host: config.host, port: config.port, configFile: CONFIG_FILE },
...
export const auditTool = defineTool({
    ...
    schema: {
        limit: z.number().int().min(1).max(500).optional().describe("How many entries to return (default 50, newest first)"),
    ...
    handler: async (args: { limit?: number; tool?: string; onlyErrors?: boolean }) => {
        try {
            ...
            const lines = entries.map(entry => {
                ...
                return `${entry.ts}  ${entry.ok ? "OK  " : "FAIL"}  ${entry.tool.padEnd(16)} ${entry.action.padEnd(11)} ${entry.target ?? ""}${detail}`;
            });
...
export const setModeTool = defineTool({
    ...
    description:
        "Change the security mode: paranoic (workspace root only), auto (root + allowed paths), bypass (no path restrictions, fully autonomous agent).",
    schema: {
...
export const workspaceUseTool = defineTool({
    name: "workspace_use",
    description: "Switch the active workspace profile, i.e. change the project root all relative paths resolve against, without restarting the server.",
    schema: {
    ...
    handler: async (args: { name: string }) => {
        try {
            ...
            if (!config.profiles.some(profile => profile.name === args.name)) {
                return fail(
                    `Профиль '${args.name}' не найден. Есть: ${config.profiles.map(profile => profile.name).join(", ")}. Создать: workspace_add.`
                );
            }

            const updated = await updateConfig(current => {
                current.activeProfile = args.name;
            });
...
export const workspaceAddTool = defineTool({
    name: "workspace_add",
    description: "Register a new workspace profile pointing at another project directory, and optionally switch to it right away.",
    schema: {
    ...
    handler: async (args: { name: string; root: string; allowedPaths?: string[]; use?: boolean }) => {
        try {
            ...
            const config = await updateConfig(current => {
                const existing = current.profiles.find(profile => profile.name === args.name);
                if (existing) {
                    existing.root = root;
                ...
                if (args.use) current.activeProfile = args.name;
            });

            await audit({ tool: "workspace_add", action: "add", target: args.name, ok: true, detail: { root } });

            return ok(
                `Профиль '${args.name}' сохранён (root: ${root}). Активный сейчас: ${config.activeProfile}`
            );
        } catch (error) {
...
export const workspaceAllowTool = defineTool({
    ...
    description: "Allow access to a directory outside the workspace root while staying in auto mode (instead of switching to bypass).",
    schema: {
