...
export const terminalOpenTool = defineTool({
    ...
    handler: async (args: { name?: string; cwd?: string; shell?: ShellKind }) => {
        try {
            ...
            await audit({ tool: "terminal_open", action: "open", target: info.id, ok: true, detail: { cwd: info.cwd, shell: info.shell } });

            return okJson(info, `Терминал открыт. Запускай команды: terminal_run { sessionId: "${info.id}", command: "..." }`);
        } catch (error) {
    ...
    handler: async (args: { sessionId: string; command: string; waitMs?: number }) => {
        try {
            ...
            await audit({
                ...
                detail: { sessionId: args.sessionId, exitCode: result.exitCode, completed: result.completed, cwd: result.cwd }
            });

            const status = result.completed
                ? `завершено за ${formatDuration(result.elapsedMs)}, exit=${result.exitCode}`
                : `ВСЁ ЕЩЁ ВЫПОЛНЯЕТСЯ (${formatDuration(result.elapsedMs)}) — читай дальше через terminal_read { sessionId: "${args.sessionId}", since: ${result.cursor} }`;
...
export const terminalReadTool = defineTool({
    ...
    schema: {
        ...
        since: z.number().int().min(0).optional().describe("Cursor from a previous terminal_run/terminal_read call: returns only newer output"),
    ...
    handler: async (args: { sessionId: string; since?: number; tail?: number; maxChars?: number }) => {
        try {
            ...
            const header =
                ...
                (info.status === "running" ? ` (${info.lastCommand} — ${formatDuration(info.runningForMs ?? 0)})` : ` lastExit=${info.lastExitCode}`) +
...
export const terminalWriteTool = defineTool({
    ...
    handler: async (args: { sessionId: string; input: string; appendNewline?: boolean }) => {
        try {
            const session = terminals.get(args.sessionId);
            session.write(args.input, args.appendNewline ?? true);
            await Bun.sleep(300);
...
export const terminalCloseTool = defineTool({
    name: "terminal_close",
    description: "Close a terminal session (kills whatever is running in it) or all sessions at once.",
    schema: {
    ...
    handler: async (args: { sessionId?: string; all?: boolean; force?: boolean }) => {
        try {
            ...
            if (!args.sessionId) {
                return fromError("Error closing terminal", new Error("Нужен sessionId или all=true"));
            }
